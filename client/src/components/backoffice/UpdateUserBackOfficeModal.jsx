...
const UpdateUserBackOfficeModal = ({ setOpen, userData, updateUser }) => {
  const [name, setName] = useState(userData.name);
  const [email, setEmail] = useState(userData.email);
  const [password, setPassword] = useState("");
  const [typeAccount, setTypeAccount] = useState(userData.typeAccount);

  function prepareUpdateUser() {
    if (updateUser(name, email, password, typeAccount, userData.userId)) {
      setOpen(false);
    }
  }
          ...
          >
            <div className="inline-block transform overflow-hidden rounded-lg bg-white bg-white px-4 pt-5 pb-4 text-left align-bottom shadow-xl transition-all dark:bg-gray-800 sm:my-8 sm:w-full sm:max-w-lg sm:p-6 sm:align-middle">
              ...
              <div className="sm:flex sm:items-start">
                ...
                <div className="mt-3 w-full text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div className="sm:col-span-3">
                    ...
                    >
                      <option value="Moderator" defaultValue>
                        Moderator
                      </option>
              ...
              <div className="mt-8 sm:mt-4 sm:flex sm:flex-row-reverse">
                <button
                  type="button"
                  className="inline-flex w-full justify-center rounded-md border border-transparent bg-yellow-600 px-4 py-2 text-base font-medium 
                                    text-white shadow-sm focus:outline-none focus:ring-green-500  focus:ring-offset-2 hover:bg-yellow-700 sm:ml-3 sm:w-auto sm:text-sm"
                  onClick={() => prepareUpdateUser()}
                >
                  Update
                </button>
                <button
...
export default UpdateUserBackOfficeModal;
