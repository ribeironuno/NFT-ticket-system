...
const EditNonSeatedModal = ({ setOpen, editSectionFunction, sectionInfo }) => {
    const [sectionName, setSectionName] = useState(sectionInfo.name || '')
    const [nameDoor, setDoorName] = useState(sectionInfo.door || '')
    const [capacity, setCapacity] = useState(sectionInfo.capacity || '')

    function editNonSeatedSection() {
        if (editSectionFunction(sectionName, nameDoor, capacity)) {
            setOpen(false)
        }
    }
                            ...
                            <div className="sm:flex sm:items-start">
                                <div className='w-full flex justify-start flex-wrap'>
                                    ...
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full md:w-fit align-middle b">
                                        <Dialog.Title as="h3" className="text-lgleading-6 font-medium text-gray-900 dark:text-gray-300 mb-8">
                                            Editing a unseated section
                                        </Dialog.Title>
                                    </div>
...
export default EditNonSeatedModal