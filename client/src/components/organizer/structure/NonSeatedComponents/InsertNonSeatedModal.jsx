...
const InsertNonSeatedModal = ({ setOpen, addSectionFunction }) => {

    const [nameDoor, setDoorName] = useState("")
    const [sectionName, setSectionName] = useState("")
    const [capacity, setCapacity] = useState('')

    function createNonSeatedSection() {
        const newSection = {
            "name": sectionName,
            "door": nameDoor,
            "capacity": capacity,
        ...
        if (addSectionFunction(newSection)) {
            setOpen(false)
        }
    }
                    ...
                    <Transition.Child
                        ...
                        <div className="inline-block bg-white dark:bg-gray-800 align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                            ...
                            <div className="mt-8 sm:mt-4 sm:flex sm:flex-row-reverse">
                                <button
                                    type="button"
                                    className="w-full mb-6 sm:mb-0 inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-sm"
                                    onClick={() => createNonSeatedSection()}>
                                    Create
                                </button>
                                <button
                                    type="button"
...
export default InsertNonSeatedModal
